...
{
	struct clock_event_device *evt = get_proxy_tick_device();
...
static void register_proxy_device(void *arg) /* irqs_disabled() */
{
	...
	/*
	 * If the proxy replaced the real device as the system tick
	 * device, the real device was shutdown, and its event handler
	 * was nop'ed.  We need to restore a valid handler routing
	 * ticks to the regular timer core as if they came from the
	 * proxy device, until the timer IRQ is pipelined. Once
	 * pipelined, ticks will be routed to the overlay handler
	 * instead.
	 */
...
static int enable_timer_pipelining(void *arg) /* hard_irqs_disabled() */
{
	const struct proxy_tick_ops *ops = arg;
	 ...
	 * device, then turn on pipelining for the associated IRQ
	 * (duplicates are silently ignored if the IRQ is common to
	 * multiple CPUs).
	 */
	...
	/*
	 * irq_set_pipelining() grabs the IRQ descriptor lock which is
	 * mutable, so that is fine to invoke this routine with hard
	 * IRQs off.
	 */
	irq_set_pipelining(real_ced->irq, true);
		       ...
		       const struct cpumask *cpumask)
{
	...
	ret = request_percpu_irq_flags(sirq, proxy_irq_handler,
				       "Proxy tick interrupt",
	 ...
	 * then turn on pipelining for the timer IRQ associated to the
	 * latter. The last two steps are performed over a
	 * stop_machine_pipelined() context, so that no tick can race
	 * with this code while we swap handlers.
	 *
	...
	/* Start ticking from the head interrupt stage. */
	stop_machine_pipelined(enable_timer_pipelining, ops, cpumask);
out:
	put_online_cpus();
...
static int disable_timer_pipelining(void *arg) /* hard_irqs_disabled() */
{
	struct clock_event_device *real_ced, *proxy_ced;
	struct proxy_tick_device *ptd;
	...
	irq_set_pipelining(real_ced->irq, false);
			  ...
			  const struct cpumask *cpumask)
{
	 ...
	 */
	stop_machine_pipelined(disable_timer_pipelining, NULL, cpu_online_mask);
...
{
	/*
	 * Schedule a tick on the proxy device to occur from the root
	 * stage, which will trigger proxy_irq_handler() at some point
	 * (i.e. when the root stage is back in control and not
	 * stalled).
	 */
	irq_stage_post_root(proxy_tick_irq);
}
