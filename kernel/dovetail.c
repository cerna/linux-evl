...
static inline
void __call_mayday(struct thread_info *ti, struct pt_regs *regs)
{
	clear_ti_thread_flag(ti, TIF_MAYDAY);
	dovetail_mayday_hook(regs);
}

static void call_mayday(struct thread_info *ti, struct pt_regs *regs)
{
	unsigned long flags;

	flags = hard_local_irq_save();
	__call_mayday(ti, regs);
	hard_local_irq_restore(flags);
}

static int do_pipeline_syscall(struct thread_info *ti,
			       struct pt_regs *regs)
{
...
{
	unsigned long local_flags = READ_ONCE(ti_local_flags);
	int ret;
	...
	if (nr >= NR_syscalls && (local_flags & _TLF_HEAD)) {
		...
		local_flags = READ_ONCE(ti_local_flags);
	...
	if ((local_flags & _TLF_DOVETAIL) || nr >= NR_syscalls) {
		ret = do_pipeline_syscall(ti, regs);
		local_flags = READ_ONCE(ti_local_flags);
		if (local_flags & _TLF_HEAD)
			return 1; /* don't pass down, no tail work. */
		if (ret)
