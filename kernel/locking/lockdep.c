...
{
	return irq_stage_disabled();
}

#ifdef CONFIG_DEBUG_HARD_SPINLOCK
/*
 * If LOCKDEP is enabled for hard and mutable locks, we want irqs to
 * be disabled for both stages when traversing the lockdep code (at
 * the expense of massive latency overhead though). Otherwise, we only
 * need the root stage to be stalled, as no locks manipulated from the
 * head stage may call into the lockdep machinery.
 */
static inline unsigned long lockdep_stage_test_and_disable(int *irqsoff)
{
	return irq_stage_test_and_disable(irqsoff);
}
...
{
	irq_stage_restore(flags);
}

#define lockdep_stage_save_flags(__flags)	irq_stage_save_flags(__flags)
...
#endif /* !CONFIG_DEBUG_HARD_SPINLOCK */
