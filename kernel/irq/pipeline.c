...
/*
 * handle_pipelined_irq() - Handles sticky interrupts over the root
 * stage, or any interrupt over the head stage.
 */
static void handle_pipelined_irq(unsigned int irq, struct irq_desc *desc)
{
	...
	for_each_action_of_desc(desc, action) {
		res = __call_action_handler(action, irq, desc);
		ret |= res;
	}

	/*
	 * The caller has to issue release_irq() to unmask a pipelined
	 * interrupt when it sees fit.
	 */
	 ...
	 * have a problem dealing with a pipelined interrupt. The
	 * error detection logic is as follows:
	 *
	 * - check and complain about any bogus return value from a
	 * pipelined IRQ handler: we only allow IRQ_HANDLED and
	...
	if (ret != IRQ_NONE) {
		printk(KERN_ERR "pipelined irq event %d: bogus return value %x\n",
				irq, ret);
	 ...
	 * IRQ line should still be in a held state, i.e. masked if
	 * level, so no IRQ storm should happen while hard IRQs are
	 * enabled back on the interrupt unwinding path.
	 *
	 ...
	 * head stage, i.e. IRQF_PIPELINED is present in the action
	 * flags.
	 *
	 * - their action handler was temporarily turned into a dual
	 * mode routine capable of handling events from both the root
	 * and head stages (see irq_set_pipelining(), tick proxying is
	 * a typical user). In this case, IRQF_PIPELINED does not
	 * appear in the action flags.
	 *
	...
	if (unlikely(desc->irqs_unhandled > 1000)) {
		printk(KERN_ERR "pipelined irq %d: stuck or unexpected\n", irq);
		raw_spin_lock_irqsave(&desc->lock, flags);
 ...
 * stage by handle_pipelined_irq() happens here too.
 */
static inline
void handle_root_irq(unsigned int irq, struct irq_desc *desc)
{
	...
	if (likely(action == NULL || !(action->flags & IRQF_STICKY)))
		...
		if (desc->istate & IRQS_SPURIOUS_DISABLED) {
			if (!WARN_ON_ONCE(irqd_irq_disabled(&desc->irq_data))) {
				printk(KERN_ERR "pipelined irq %d: disabling\n", irq);
				disable_irq_nosync(irq);
			}
		} else {
			irq_enter();
			handle_pipelined_irq(irq, desc);
			irq_exit();
		}
	}
