		       ...
		       struct cpuidle_state *state)
{
	...
	/*
	 * We may go idle if no interrupt is pending for delivery to
	 * the root stage.
	 */
	hard_local_irq_disable();
	p = irq_root_this_context();
	 ...
	 * idle. If we may enter the idle state, leave this routine
	 * with hard irqs off to prevent any event from sneaking into
	 * the interrupt log we have just checked.
	 */
	if (!irq_staged_waiting(p) && irq_cpuidle_control(dev, state))
		return true;

	/*
	 * Otherwise, restore hard irqs on and unstall the root stage.
	 */
