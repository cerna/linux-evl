...
static struct cpumask smp_stop_map;
static struct cpumask smp_pass_map;
static unsigned long smp_stop_wait;
static DEFINE_HARD_SPINLOCK(smp_barrier);
static atomic_t smp_stop_count = ATOMIC_INIT(0);
...
static irqreturn_t pipeline_sync_handler(int irq, void *dev_id)
{
	...
	/*
	 * We are now in sync with the stop requestor running on a
	 * remote CPU. Enter a spinning wait until the barrier is
	 * released.
	 */
...
static struct irqaction stop_ipi = {
...
trace_on_debug void __root_irq_restore_nosync(unsigned long x)
{
...
void irq_stage_push(struct irq_stage *stage, const char *name)
{
...
/**
 *	irq_pipeline_stop_many - Enter an inter-CPU critical section
 ...
 *      spinning in a tight loop until irq_pipeline_resume() is called
				     ...
				     cpu_stop_fn_t fn, void *data)
{
	...
	if (!cpumask_test_and_set_cpu(cpu, &smp_stop_map)) {
		/*
		 * Wait for an ongoing locking sequence to end before
		 * starting a new one.
		 */
		while (test_and_set_bit(0, &smp_stop_wait)) {
			n = 0;
			hard_local_irq_enable();
			do
				cpu_relax();
...
restart:
	...
	atomic_inc(&smp_stop_count);

#endif	/* CONFIG_SMP */

	return flags;
}
EXPORT_SYMBOL_GPL(irq_pipeline_stop_many);

/**
 *	irq_pipeline_resume - Exit an inter-CPU critical section
 *	@flags:		Hardware interrupt state to restore
 *
 *      This routine exits a critical section started by a call to
 *	irq_pipeline_stop_many().  Hardware interrupts are restored in
 *	the current CPU.
 */
void irq_pipeline_resume(unsigned long flags)
{
	/*
	 * CPUs cannot be unplugged until we release the pipeline, so
	 * checking num_online_cpus() is fine.
	 */
	if (num_online_cpus() == 1) {
		hard_local_irq_restore(flags);
		return;
...
#ifdef CONFIG_SMP
	if (atomic_dec_and_test(&smp_stop_count)) {
		raw_spin_unlock(&smp_barrier);
		while (!cpumask_empty(&smp_sync_map))
			cpu_relax();
		cpumask_clear_cpu(raw_smp_processor_id(), &smp_stop_map);
		clear_bit(0, &smp_stop_wait);
...
EXPORT_SYMBOL_GPL(irq_pipeline_resume);
