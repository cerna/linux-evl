 ...
 *
 *      When pipelined, some controllers may need in-band percpu IRQs
 *      to be masked so that we don't get caught into an interrupt
 *      storm, waiting for the IRQ log to be synced after hard irqs
...
void handle_percpu_irq(struct irq_desc *desc)
{
	...
	if (on_pipeline_entry()) {
		...
		if (chip->irq_eoi)
			chip->irq_eoi(&desc->irq_data);
		if (!handled && desc->irq_data.chip->irq_mask)
			desc->irq_data.chip->irq_mask(&desc->irq_data);
		return;
	}
