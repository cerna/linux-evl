		       ...
		       const struct compat_sched_param_ex __user *u_cp)
{
	...
	if (u_cp == NULL || steely_copy_from_user(&cpex, u_cp, sizeof(cpex)))
		return -EFAULT;
		       ...
		       const struct sched_param_ex *p)
{
	...
	return steely_copy_to_user(u_cp, &cpex, sizeof(cpex));
}
		     ...
		     const struct compat_mq_attr __user *u_cap)
{
	...
	if (u_cap == NULL ||
	    steely_copy_from_user(&cattr, u_cap, sizeof(cattr)))
		return -EFAULT;
		     ...
		     const struct mq_attr *ap)
{
	...
	return u_cap == NULL ? -EFAULT :
		steely_copy_to_user(u_cap, &cattr, sizeof(cattr));
}
		       ...
		       const struct compat_sigevent *__user u_cev)
{
	...
	ret = steely_copy_from_user(&cev, u_cev, sizeof(cev));
	if (ret)
		return ret;
...
int sys32_get_sigset(sigset_t *set, const compat_sigset_t *u_cset)
{
	...
	ret = steely_copy_from_user(&cset, u_cset, sizeof(cset));
	if (ret)
		return ret;
...
int sys32_put_sigset(compat_sigset_t *u_cset, const sigset_t *set)
{
	...
	return steely_copy_to_user(u_cset, &cset, sizeof(cset));
}
...
int sys32_get_sigval(union sigval *val, const union compat_sigval *u_cval)
{
	...
	ret = steely_copy_from_user(&cval, u_cval, sizeof(cval));
	if (ret)
		return ret;
		    ...
		    int ciovlen)
{
	...
	for (n = 0, p = u_ciov; n < ciovlen; n++, p++) {
		ret = steely_copy_from_user(&ciov, p, sizeof(ciov));
		if (ret)
			return ret;
		    ...
		    int iovlen)
{
	...
	for (n = 0, p = u_ciov; n < iovlen; n++, p++) {
		...
		ret = steely_copy_to_user(p, &ciov, sizeof(*p));
		if (ret)
			return ret;
	}
