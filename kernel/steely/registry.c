...
static DEFINE_PER_CPU(struct irq_work, proc_irq_work);
...
((n) < sizeof(primes) / sizeof(int) ? \
 (n) : sizeof(primes) / sizeof(int) - 1)
	...
	return primes[obj_hash_max(CONFIG_STEELY_REGISTRY_NRSLOTS / 100)];
}
...
static void registry_proc_schedule(struct irq_work *work)
{
	/*
...
static inline void registry_export_pnode(struct xnobject *object,
					 struct xnpnode *pnode) /* hard_irqs_disabled() */
{
	object->vfilp = XNOBJECT_PNODE_RESERVED1;
	object->pnode = pnode;
	list_del(&object->link);
	list_add_tail(&object->link, &proc_object_list);
	irq_work_queue(&proc_irq_work);
}

static inline void registry_unexport_pnode(struct xnobject *object) /* hard_irqs_disabled() */
{
	if (object->vfilp != XNOBJECT_PNODE_RESERVED1) {
		/*
...
{
	int n, ret __maybe_unused, cpu __maybe_unused;

	registry_obj_slots = kmalloc(CONFIG_STEELY_REGISTRY_NRSLOTS *
				     sizeof(struct xnobject), GFP_KERNEL);
	if (registry_obj_slots == NULL)
		return -ENOMEM;

#ifdef CONFIG_STEELY_VFILE
	ret = xnvfile_init_dir("registry", &registry_vfroot, &steely_vfroot);
	if (ret)
		return ret;
	...
	for_each_possible_cpu(cpu)
		init_irq_work(&per_cpu(proc_irq_work, cpu),
			      registry_proc_schedule);
#endif /* CONFIG_STEELY_VFILE */

	next_object_stamp = 0;

	for (n = 0; n < CONFIG_STEELY_REGISTRY_NRSLOTS; n++) {
