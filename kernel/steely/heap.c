...
{
	xnvfile_init_snapshot("heap", &vfile, &steely_vfroot);
}
...
void *xnheap_alloc(struct xnheap *heap, u32 size)
{
	...
	if (likely(size <= XNHEAP_PAGESZ * 2)) {
		...
		if (block == NULL) {
			...
			STEELY_BUG_ON(STEELY, (caddr_t)block < heap->membase ||
...
void xnheap_free(struct xnheap *heap, void *block)
{
	...
	default:
		...
		STEELY_BUG_ON(STEELY, heap->buckets[ilog].fcount < 0);
...
void *xnheap_vmalloc(size_t size)
{
	 ...
	 * contiguous.
	 *
	 * 32bit systems which would need HIGHMEM for running a Steely
