...
static DEFINE_PER_CPU(struct irq_work, wakeup_work);
...
{
	irq_work_queue(raw_cpu_ptr(&wakeup_work));
}
...
static void *xnpipe_default_alloc_ibuf(size_t size, void *xstate)
{
	...
	if (size > xnheap_get_size(&steely_heap))
...
int xnpipe_disconnect(int minor)
{
	...
	if (state->status & XNPIPE_USER_WREAD) {
		 ...
		 * operation from the Steely side (read/write or
		 * poll).
		 */
		state->status |= XNPIPE_USER_WREAD_READY;
		need_sched = 1;
			   ...
			   char *buf, size_t count, loff_t *ppos)
{
	 ...
	 * including from the Steely side.
	 */
...
retry:
	...
	/* Wake up a Steely sleeper if any. */
...
static unsigned xnpipe_poll(struct file *file, poll_table *pt)
{
	...
	if (!list_empty(&state->outq))
		 ...
		 * silently unlinked the next time the Steely side
...
int xnpipe_mount(void)
{
	...
	if (IS_ERR(xnpipe_class)) {
		printk(STEELY_ERR "error creating rtpipe class, err=%ld\n",
	...
	for_each_possible_cpu(cpu)
		init_irq_work(&per_cpu(wakeup_work, cpu),
			      xnpipe_wakeup_proc);
