 ...
 * for this (i.e. CONFIG_STEELY_DEBUG_TRACE_RELAX).
 *
 ...
 * made available through the /proc/steely/debug/relax vfile.
 *
 ...
 * seen from userland (via the internal sc_steely_backtrace
...
{
	u32 size = CONFIG_STEELY_DEBUG_TRACE_LOGSZ * 1024;
	void *p;
	int ret;
	...
	ret = xnvfile_init_regular("relax", &relax_vfile, &steely_debug_vfroot);
	if (ret) {
...
#else /* !CONFIG_STEELY_DEBUG_TRACE_RELAX */
...
#endif /* !STEELY_DEBUG_TRACE_RELAX */
		       ...
		       const char *file, int line, const char *function)
{
	...
	if (unlikely(lock->owner != cpu)) {
		printk(STEELY_ERR "lock %p already unlocked on CPU #%d\n"
