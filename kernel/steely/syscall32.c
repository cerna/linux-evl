...
#include <steely/rtdm/internal.h>
#include "internal.h"
#include "syscall32.h"
#include "thread.h"
#include "mutex.h"
...
STEELY_SYSCALL32emu(thread_create, init,
		    (compat_ulong_t pth,
		     int policy,
...
STEELY_SYSCALL32emu(thread_setschedparam_ex, conforming,
		    (compat_ulong_t pth,
		     int policy,
		     const struct compat_sched_param_ex __user *u_param_ex,
		     __u32 __user *u_winoff,
...
STEELY_SYSCALL32emu(thread_getschedparam_ex, current,
		    (compat_ulong_t pth,
		     int __user *u_policy,
...
STEELY_SYSCALL32emu(sem_open, lostage,
		     ...
		     int oflags, mode_t mode, unsigned int value))
{
	...
	usm = __steely_sem_open(compat_ptr(cusm), u_name, oflags, mode, value);
	if (IS_ERR(usm))
		return PTR_ERR(usm);
...
STEELY_SYSCALL32emu(sem_timedwait, primary,
		    (struct steely_sem_shadow __user *u_sem,
		     struct compat_timespec __user *u_ts))
{
	return __steely_sem_timedwait(u_sem, u_ts, sys32_fetch_timeout);
}

STEELY_SYSCALL32emu(clock_getres, current,
		    (clockid_t clock_id,
		     struct compat_timespec __user *u_ts))
{
	struct timespec ts;
...
STEELY_SYSCALL32emu(clock_gettime, current,
		    (clockid_t clock_id,
		     struct compat_timespec __user *u_ts))
{
	struct timespec ts;
...
STEELY_SYSCALL32emu(clock_settime, current,
		    (clockid_t clock_id,
...
STEELY_SYSCALL32emu(clock_nanosleep, nonrestartable,
...
STEELY_SYSCALL32emu(mutex_timedlock, primary,
		    (struct steely_mutex_shadow __user *u_mx,
		     const struct compat_timespec __user *u_ts))
{
	return __steely_mutex_timedlock_break(u_mx, u_ts, sys32_fetch_timeout);
}

STEELY_SYSCALL32emu(cond_wait_prologue, nonrestartable,
		    (struct steely_cond_shadow __user *u_cnd,
		     ...
		     struct compat_timespec __user *u_ts))
{
	return __steely_cond_wait_prologue(u_cnd, u_mx, u_err, u_ts,
					   timed ? sys32_fetch_timeout : NULL);
}

STEELY_SYSCALL32emu(mq_open, lostage,
...
STEELY_SYSCALL32emu(mq_getattr, current,
...
STEELY_SYSCALL32emu(mq_timedsend, primary,
...
STEELY_SYSCALL32emu(mq_timedreceive, primary,
		     ...
		     const struct compat_timespec __user *u_ts))
{
	...
	ret = steely_copy_from_user(&clen, u_len, sizeof(*u_len));
	if (ret)
		return ret;
	...
	return ret ?: steely_copy_to_user(u_len, &clen, sizeof(*u_len));
}
...
STEELY_SYSCALL32emu(mq_notify, primary,
		    (mqd_t fd, const struct compat_sigevent *__user u_cev))
{
	...
	return __steely_mq_notify(fd, u_cev ? &sev : NULL);
}

STEELY_SYSCALL32emu(sched_weightprio, current,
		    (int policy,
...
sys32_fetch_config(int policy, const void __user *u_config, size_t *len)
{
	...
	ret = steely_copy_from_user(cbuf, u_config, *len);
	if (ret) {
		buf = ERR_PTR(ret);
		goto out;
	}
...
STEELY_SYSCALL32emu(sched_setconfig_np, conforming,
		    (int cpu, int policy,
...
STEELY_SYSCALL32emu(sched_getconfig_np, conformin,
		    (int cpu, int policy,
		     union compat_sched_config __user *u_config,
		     size_t len))
{
...
STEELY_SYSCALL32emu(sched_setscheduler_ex, conforming,
		    (compat_pid_t pid,
		     int policy,
		     const struct compat_sched_param_ex __user *u_param_ex,
		     __u32 __user *u_winoff,
...
STEELY_SYSCALL32emu(sched_getscheduler_ex, current,
		    (compat_pid_t pid,
		     int __user *u_policy,
		     struct compat_sched_param_ex __user *u_param))
{
...
STEELY_SYSCALL32emu(timer_create, current,
		    (clockid_t clock,
...
STEELY_SYSCALL32emu(timer_settime, primary,
...
STEELY_SYSCALL32emu(timer_gettime, current,
...
STEELY_SYSCALL32emu(timerfd_settime, primary,
		    (int fd, int flags,
...
STEELY_SYSCALL32emu(timerfd_gettime, current,
...
STEELY_SYSCALL32emu(sigwait, primary,
...
STEELY_SYSCALL32emu(sigtimedwait, nonrestartable,
		    (const compat_sigset_t __user *u_set,
		     ...
		     const struct compat_timespec __user *u_timeout))
{
	...
	return __steely_sigtimedwait(&set, &timeout, u_si, sys32_put_siginfo);
}

STEELY_SYSCALL32emu(sigwaitinfo, nonrestartable,
		    (const compat_sigset_t __user *u_set,
		     struct compat_siginfo __user *u_si))
{
	sigset_t set;
	...
	return __steely_sigwaitinfo(&set, u_si, sys32_put_siginfo);
}

STEELY_SYSCALL32emu(sigpending, primary, (compat_old_sigset_t __user *u_set))
{
...
STEELY_SYSCALL32emu(sigqueue, conforming,
		    (pid_t pid, int sig,
...
STEELY_SYSCALL32emu(monitor_wait, nonrestartable,
...
STEELY_SYSCALL32emu(event_wait, primary,
...
STEELY_SYSCALL32emu(select, nonrestartable,
		    (int nfds,
		     ...
		     struct compat_timeval __user *u_tv))
{
	...
	if (selector == NULL) {
		...
		if (!__steely_first_fd_valid_p(in_fds, nfds))
			return -EBADF;
		...
		err = __steely_select_bind_all(selector, in_fds, nfds);
		if (err)
			return err;
	}
	...
	do {
		...
		if (err == -ECHRNG) {
			int err = __steely_select_bind_all(selector, out_fds, nfds);
			if (err)
				return err;
		}
	} while (err == -ECHRNG);
...
STEELY_SYSCALL32emu(recvmsg, handover,
...
STEELY_SYSCALL32emu(sendmsg, handover,
...
STEELY_SYSCALL32emu(mmap, lostage,
		     ...
		     compat_uptr_t __user *u_caddrp))
{
	...
	return steely_copy_to_user(u_caddrp, &u_caddr, sizeof(u_caddr));
}

STEELY_SYSCALL32emu(backtrace, current,
		     ...
		     int reason))
{
	...
	ret = steely_copy_from_user(cbacktrace, u_backtrace,
...
#ifdef STEELY_SYSCALL32x

STEELY_SYSCALL32x(mq_timedreceive, primary,
		  (mqd_t uqd, void __user *u_buf,
		   compat_ssize_t __user *u_len,
		   unsigned int __user *u_prio,
