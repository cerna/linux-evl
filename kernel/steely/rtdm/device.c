...
static int register_driver(struct rtdm_driver *drv)
{
	...
	case RTDM_PROTOCOL_DEVICE:
		break;
	default:
		printk(STEELY_WARNING "%s has invalid device type (%#x)\n",
	...
	if (drv->device_count <= 0 ||
		...
		printk(STEELY_WARNING "%s has invalid device count (%d)\n",
	...
	if (drv->base_minor < 0 ||
		...
		printk(STEELY_WARNING "%s has invalid base minor (%d)\n",
	...
	ret = alloc_chrdev_region(&rdev, drv->base_minor, drv->device_count,
				  drv->profile_info.name);
	if (ret) {
		printk(STEELY_WARNING "cannot allocate chrdev region %s[%d..%d]\n",
		       drv->profile_info.name, drv->base_minor,
	...
	if (ret) {
		printk(STEELY_WARNING "cannot create cdev series for %s\n",
		       drv->profile_info.name);
		goto fail_cdev;
	}
	...
	steely_add_state_chain(&drv->nb_statechange);
...
{
	STEELY_BUG_ON(STEELY, drv->profile_info.magic != RTDM_CLASS_MAGIC);

	if (!atomic_dec_and_test(&drv->refcount))
		return;

	steely_remove_state_chain(&drv->nb_statechange);
...
int rtdm_dev_register(struct rtdm_device *dev)
{
	...
	trace_steely_device_register(dev);
...
void rtdm_dev_unregister(struct rtdm_device *dev)
{
	...
	trace_steely_device_unregister(dev);
...
int __init rtdm_init(void)
{
	...
	if (IS_ERR(rtdm_class)) {
		printk(STEELY_ERR "cannot create RTDM sysfs class\n");
