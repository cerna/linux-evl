...
struct steely_named_sem {
	struct steely_sem *sem;
	struct steely_sem_shadow __user *usem;
	unsigned int refs;
	struct xnid id;
};
...
static inline struct steely_resources *sem_kqueue(struct steely_sem *sem)
{
	...
	return steely_current_resources(pshared);
}

static inline int sem_check(struct steely_sem *sem)
{
	if (sem == NULL || sem->magic != STEELY_SEM_MAGIC)
		return -EINVAL;
...
int __steely_sem_destroy(xnhandle_t handle)
{
	...
	if (!steely_obj_active(sem, STEELY_SEM_MAGIC, typeof(*sem))) {
		ret = -EINVAL;
		goto fail;
	}
	...
	steely_mark_deleted(sem);
	xnregistry_remove(sem->resnode.handle);
	if (!sem->pathname)
		steely_del_resource(&sem->resnode);
	...
	steely_umm_free(&steely_ppd_get(!!(sem->flags & SEM_PSHARED))->umm,
			sem->state);
...
struct steely_sem *
__steely_sem_init(const char *name, struct steely_sem_shadow *sm,
		  int flags, unsigned int value)
{
	struct steely_sem_state *state;
	struct steely_sem *sem, *osem;
	...
	sys_ppd = steely_ppd_get(pshared);
	state = steely_umm_alloc(&sys_ppd->umm, sizeof(*state));
	if (state == NULL) {
		ret = -EAGAIN;
		goto err_free_sem;
	}
	...
	semq = &steely_current_resources(pshared)->semq;
	if ((sm->magic == STEELY_SEM_MAGIC && !list_empty(semq)) ||
	    sm->magic == STEELY_NAMED_SEM_MAGIC) {
		osem = xnregistry_lookup(sm->handle, NULL);
		if (steely_obj_active(osem, STEELY_SEM_MAGIC, typeof(*osem))) {
			ret = -EBUSY;
			goto err_lock_put;
		}
	}
	...
	sem->magic = STEELY_SEM_MAGIC;
	if (!name)
		steely_add_resource(&sem->resnode, sem, pshared);
	else
	...
	sm->magic = name ? STEELY_NAMED_SEM_MAGIC : STEELY_SEM_MAGIC;
	sm->handle = sem->resnode.handle;
	sm->state_offset = steely_umm_offset(&sys_ppd->umm, state);
	if (flags & SEM_PSHARED)
		sm->state_offset = -sm->state_offset;
	xnlock_put_irqrestore(&nklock, s);

	trace_steely_psem_init(name ?: "anon",
...
err_lock_put:
	xnlock_put_irqrestore(&nklock, s);
	steely_umm_free(&sys_ppd->umm, state);
err_free_sem:
	xnfree(sem);
out:
	trace_steely_psem_init_failed(name ?: "anon", flags, value, ret);

	return ERR_PTR(ret);
}

static int sem_destroy(struct steely_sem_shadow *sm)
{
	struct steely_sem *sem;
	int warn, ret;
	spl_t s;
	...
	steely_mark_deleted(sm);

	xnlock_put_irqrestore(&nklock, s);

	ret = __steely_sem_destroy(sem->resnode.handle);
...
static inline int do_trywait(struct steely_sem *sem)
{
	int ret;
						...
						const void __user *u_ts))
{
	...
	handle = steely_get_handle_from_user(&u_sem->handle);
	trace_steely_psem_timedwait(handle);
...
STEELY_SYSCALL(sem_init, current,
	       (struct steely_sem_shadow __user *u_sem,
		int flags, unsigned int value))
{
	struct steely_sem_shadow sm;
	struct steely_sem *sem;

	if (steely_copy_from_user(&sm, u_sem, sizeof(sm)))
		return -EFAULT;
	...
	sem = __steely_sem_init(NULL, &sm, flags, value);
	if (IS_ERR(sem))
		return PTR_ERR(sem);

	return steely_copy_to_user(u_sem, &sm, sizeof(*u_sem));
}

STEELY_SYSCALL(sem_post, current,
	       (struct steely_sem_shadow __user *u_sem))
{
	xnhandle_t handle;

	handle = steely_get_handle_from_user(&u_sem->handle);
	trace_steely_psem_post(handle);

	return sem_post(handle);
}

STEELY_SYSCALL(sem_wait, primary,
	       (struct steely_sem_shadow __user *u_sem))
{
	xnhandle_t handle;

	handle = steely_get_handle_from_user(&u_sem->handle);
	trace_steely_psem_wait(handle);

	return sem_wait(handle);
}

STEELY_SYSCALL(sem_timedwait, primary,
	       (struct steely_sem_shadow __user *u_sem,
		struct timespec __user *u_ts))
{
	return __steely_sem_timedwait(u_sem, u_ts, sem_fetch_timeout);
}

STEELY_SYSCALL(sem_trywait, primary,
	       (struct steely_sem_shadow __user *u_sem))
{
	struct steely_sem *sem;
	xnhandle_t handle;
	...
	handle = steely_get_handle_from_user(&u_sem->handle);
	trace_steely_psem_trywait(handle);
...
STEELY_SYSCALL(sem_getvalue, current,
	       (struct steely_sem_shadow __user *u_sem,
		int __user *u_sval))
{
	int ret, sval = -1;
	...
	trace_steely_psem_getvalue(handle, sval);
	if (ret)
		return ret;

	return steely_copy_to_user(u_sval, &sval, sizeof(sval));
}

STEELY_SYSCALL(sem_destroy, current,
	       (struct steely_sem_shadow __user *u_sem))
{
	struct steely_sem_shadow sm;
	int err;
	...
	trace_steely_psem_destroy(sm.handle);

	err = sem_destroy(&sm);
	if (err < 0)
		return err;

	return steely_copy_to_user(u_sem, &sm, sizeof(*u_sem)) ?: err;
}

STEELY_SYSCALL(sem_broadcast_np, current,
	       (struct steely_sem_shadow __user *u_sem))
{
	struct steely_sem *sem;
	xnhandle_t handle;
	...
	handle = steely_get_handle_from_user(&u_sem->handle);
	trace_steely_psem_broadcast(u_sem->handle);
...
STEELY_SYSCALL(sem_inquire, current,
	       (struct steely_sem_shadow __user *u_sem,
		...
		size_t waitsz))
{
	...
	handle = steely_get_handle_from_user(&u_sem->handle);
	trace_steely_psem_inquire(handle);
...
void steely_sem_reclaim(struct steely_resnode *node, spl_t s)
{
	struct steely_sem *sem;
	xnhandle_t handle;
	int named, ret;

	sem = container_of(node, struct steely_sem, resnode);
	named = (sem->flags & SEM_NAMED) != 0;
	handle = node->handle;
	xnlock_put_irqrestore(&nklock, s);
	ret = __steely_sem_destroy(handle);
	if (named && ret == -EBUSY)
		xnregistry_unlink(xnregistry_key(handle));
}

static struct steely_named_sem *
sem_search(struct steely_process *process, xnhandle_t handle)
{
	struct xnid *i;
	...
	return container_of(i, struct steely_named_sem, id);
}

static struct steely_sem_shadow __user *
sem_open(struct steely_process *process,
	 struct steely_sem_shadow __user *ushadow,
	 struct filename *filename, int oflags, mode_t mode,
	 unsigned int value)
{
	const char *name = filename->name;
	struct steely_sem_shadow shadow;
	struct steely_named_sem *u, *v;
	struct steely_sem *sem;
	xnhandle_t handle;
	spl_t s;
	int rc;
  ...
  retry_bind:
	...
	case 0:
		...
		xnlock_get_irqsave(&nklock, s);
		sem = xnregistry_lookup(handle, NULL);
		if (sem && sem->magic != STEELY_SEM_MAGIC) {
			xnlock_put_irqrestore(&nklock, s);
			return ERR_PTR(-EINVAL);
		}
	...
	case -EWOULDBLOCK:
		...
		shadow.magic = 0;
		sem = __steely_sem_init(&name[1], &shadow,
		...
		if (steely_copy_to_user(ushadow, &shadow, sizeof(shadow))) {
			__steely_sem_destroy(shadow.handle);
	...
	trace_steely_psem_open(name, handle, oflags, mode, value);

	return u->usem;
}

static int sem_close(struct steely_process *process, xnhandle_t handle)
{
	struct steely_named_sem *u;
	spl_t s;
	int err;
		  ...
		  int oflags, mode_t mode, unsigned int value)
{
	...
	if (IS_ERR(usm)) {
		trace_steely_psem_open_failed(filename->name, oflags, mode,
					      value, PTR_ERR(usm));
		putname(filename);
	}
...
STEELY_SYSCALL(sem_open, lostage,
		...
		int oflags, mode_t mode, unsigned int value))
{
	...
	usm = __steely_sem_open(usm, u_name, oflags, mode, value);
	if (IS_ERR(usm))
		return PTR_ERR(usm);
...
STEELY_SYSCALL(sem_close, lostage,
	       (struct steely_sem_shadow __user *usm))
{
	struct steely_process *process;
	xnhandle_t handle;
	...
	handle = steely_get_handle_from_user(&usm->handle);
	trace_steely_psem_close(handle);
...
static inline int sem_unlink(const char *name)
{
	...
	if (__steely_sem_destroy(handle) == -EBUSY)
		xnregistry_unlink(xnregistry_key(handle));

	return 0;
}

STEELY_SYSCALL(sem_unlink, lostage,
	       (const char __user *u_name))
{
	struct filename *filename;
	int ret;
	...
	trace_steely_psem_unlink(filename->name);
...
static void reclaim_named_sem(void *arg, struct xnid *i)
{
	...
	u = container_of(i, struct steely_named_sem, id);
	u->refs = 1;
