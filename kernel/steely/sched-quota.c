 ...
 * from the runqueue.
 *
 * Updating the remaining budget is done each time the Steely core
 ...
 * a result of this, the Steely core will ask for the next thread to
 ...
 * Steely core. This only means that the SCHED_QUOTA policy won't pick
 * them until the corresponding budget is replenished.
 */
static DECLARE_BITMAP(group_map, CONFIG_STEELY_SCHED_QUOTA_NR_GROUPS);
...
static void quota_refill_handler(struct xntimer *timer)
{
	...
	STEELY_BUG_ON(STEELY, list_empty(&qs->groups));
				 ...
				 const union xnsched_policy_param *p)
{
	...
	tgid = p->quota.tgid;
	if (tgid < 0 || tgid >= CONFIG_STEELY_SCHED_QUOTA_NR_GROUPS)
		return -EINVAL;
...
int xnsched_quota_create_group(struct xnsched_quota_group *tg,
			       ...
			       int *quota_sum_r)
{
	int tgid, nr_groups = CONFIG_STEELY_SCHED_QUOTA_NR_GROUPS;
	struct xnsched_quota *qs = &sched->quota;
