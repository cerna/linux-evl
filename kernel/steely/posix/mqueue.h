...
#ifndef _STEELY_POSIX_MQUEUE_H
#define _STEELY_POSIX_MQUEUE_H
...
STEELY_SYSCALL_DECL(mq_open,
		    (const char __user *u_name, int oflags,
		     mode_t mode, struct mq_attr __user *u_attr));

STEELY_SYSCALL_DECL(mq_close, (mqd_t uqd));

STEELY_SYSCALL_DECL(mq_unlink, (const char __user *u_name));

STEELY_SYSCALL_DECL(mq_getattr, (mqd_t uqd, struct mq_attr __user *u_attr));

STEELY_SYSCALL_DECL(mq_timedsend,
		    (mqd_t uqd, const void __user *u_buf, size_t len,
		     unsigned int prio, const struct timespec __user *u_ts));

STEELY_SYSCALL_DECL(mq_timedreceive,
		    (mqd_t uqd, void __user *u_buf, ssize_t __user *u_len,
		     unsigned int __user *u_prio,
		     const struct timespec __user *u_ts));

STEELY_SYSCALL_DECL(mq_notify,
		    (mqd_t fd, const struct sigevent *__user evp));

#endif /* !_STEELY_POSIX_MQUEUE_H */
