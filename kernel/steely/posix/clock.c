...
static int do_clock_host_realtime(struct timespec *tp)
{
	 ...
	 * ensures that a reader (on the Steely side) cannot interrupt
	 * a writer (on the Linux kernel side) on the same CPU.  The
	 * urw block is required when a reader is interleaved by a
	 * writer on a different CPU. This follows the approach from
	 * userland, where taking the spinlock is not possible.
	 */
