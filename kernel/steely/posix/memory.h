 ...
 */
#ifndef _STEELY_POSIX_MEMORY_H
#define _STEELY_POSIX_MEMORY_H

#include <steely/ppd.h>

#define steely_umm_set_name(__umm, __fmt, __args...)	\
	xnheap_set_name(&(__umm)->heap, (__fmt), ## __args)

static inline
void *steely_umm_alloc(struct steely_umm *umm, __u32 size)
{
	return xnheap_alloc(&umm->heap, size);
}

static inline
void *steely_umm_zalloc(struct steely_umm *umm, __u32 size)
{
	return xnheap_zalloc(&umm->heap, size);
}

static inline
void steely_umm_free(struct steely_umm *umm, void *p)
{
	xnheap_free(&umm->heap, p);
}

static inline
__u32 steely_umm_offset(struct steely_umm *umm, void *p)
{
