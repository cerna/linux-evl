...
void steely_thread_map(struct xnthread *curr)
{
	...
	thread = container_of(curr, struct steely_thread, threadbase);
	thread->process = steely_current_process();
	STEELY_BUG_ON(STEELY, thread->process == NULL);
}
...
struct xnthread_personality *steely_thread_exit(struct xnthread *curr)
{
	...
	xnlock_get_irqsave(&nklock, s);
	steely_mark_deleted(thread);
	list_del(&thread->next);
	xnlock_put_irqrestore(&nklock, s);
	steely_signal_flush(thread);
...
struct xnthread_personality *steely_thread_finalize(struct xnthread *zombie)
{
	...
	thread = container_of(zombie, struct steely_thread, threadbase);
	xnfree(thread);
			   ...
			   int xid, __u32 __user *u_winoff)
{
	...
	ret = steely_map_user(&thread->threadbase, u_winoff);
	if (ret) {
