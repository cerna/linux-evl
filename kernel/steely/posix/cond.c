...
pthread_cond_init(struct steely_cond_shadow *cnd, const struct steely_condattr *attr)
{
	...
	struct steely_cond *cond, *old_cond;
	...
	sys_ppd = steely_ppd_get(attr->pshared);
	...
	condq = &steely_current_resources(attr->pshared)->condq;
	if (cnd->magic == STEELY_COND_MAGIC && !list_empty(condq)) {
		old_cond = xnregistry_lookup(cnd->handle, NULL);
		if (steely_obj_active(old_cond, STEELY_COND_MAGIC,
				      typeof(*old_cond))) {
			ret = -EBUSY;
			goto fail_register;
		}
	...
	cond->magic = STEELY_COND_MAGIC;
	xnsynch_init(&cond->synchbase, synch_flags, NULL);
	cond->attr = *attr;
	cond->mutex = NULL;
	steely_add_resource(&cond->resnode, cond, attr->pshared);

	cnd->handle = cond->resnode.handle;
	cnd->state_offset = steely_umm_offset(&sys_ppd->umm, state);
	cnd->magic = STEELY_COND_MAGIC;
...
static inline int pthread_cond_destroy(struct steely_cond_shadow *cnd)
{
	...
	if (!steely_obj_active(cnd, STEELY_COND_MAGIC, struct steely_cond_shadow)
	    || !steely_obj_active(cond, STEELY_COND_MAGIC, struct steely_cond)) {
		xnlock_put_irqrestore(&nklock, s);
		return -EINVAL;
	}

	if (cond->resnode.scope !=
	    steely_current_resources(cond->attr.pshared)) {
		xnlock_put_irqrestore(&nklock, s);
		return -EPERM;
	}
	...
	steely_cond_reclaim(&cond->resnode, s); /* drops lock */

	steely_mark_deleted(cnd);
						 ...
						 ktime_t abs_to)
{
	...
	if (!steely_obj_active(cond, STEELY_COND_MAGIC, struct steely_cond)
	...
	/* Unlock mutex. */
	err = steely_mutex_release(cur, mutex);
	if (err < 0)
		goto unlock_and_return;
						 ...
						 struct steely_mutex *mutex)
{
	...
	err = __steely_mutex_acquire_unchecked(cur, mutex, NULL);
	if (err == -EINTR)
		goto unlock_and_return;
...
STEELY_SYSCALL(cond_init, current,
	       (struct steely_cond_shadow __user *u_cnd,
		const struct steely_condattr __user *u_attr))
{
	struct steely_cond_shadow cnd;
	struct steely_condattr attr;
	int err;

	if (steely_copy_from_user(&cnd, u_cnd, sizeof(cnd)))
		return -EFAULT;

	if (steely_copy_from_user(&attr, u_attr, sizeof(attr)))
		return -EFAULT;

	trace_steely_cond_init(u_cnd, &attr);

	err = pthread_cond_init(&cnd, &attr);
	if (err < 0)
		return err;

	return steely_copy_to_user(u_cnd, &cnd, sizeof(*u_cnd));
}

STEELY_SYSCALL(cond_destroy, current,
	       (struct steely_cond_shadow __user *u_cnd))
{
	struct steely_cond_shadow cnd;
	int err;
	...
	trace_steely_cond_destroy(u_cnd);
						     ...
						     const void __user *u_ts))
{
	...
	handle = steely_get_handle_from_user(&u_cnd->handle);
	...
	if (fetch_timeout) {
		...
		err = steely_cond_timedwait_prologue(cur, cond, mx, XN_INFINITE);
	}
	...
	case -ETIMEDOUT:
		perr = d.err = err;
		err = steely_cond_timedwait_epilogue(cur, cond, mx);
		break;
...
STEELY_SYSCALL(cond_wait_prologue, nonrestartable,
	       (struct steely_cond_shadow __user *u_cnd,
		struct steely_mutex_shadow __user *u_mx,
		int *u_err,
		unsigned int timed,
...
STEELY_SYSCALL(cond_wait_epilogue, primary,
	       (struct steely_cond_shadow __user *u_cnd,
		struct steely_mutex_shadow __user *u_mx))
{
	struct xnthread *cur = xnthread_current();
...
void steely_cond_reclaim(struct steely_resnode *node, spl_t s)
{
	struct steely_cond *cond;

	cond = container_of(node, struct steely_cond, resnode);
	xnregistry_remove(node->handle);
	steely_del_resource(node);
	xnsynch_destroy(&cond->synchbase);
	steely_mark_deleted(cond);
	xnlock_put_irqrestore(&nklock, s);

	steely_umm_free(&steely_ppd_get(cond->attr.pshared)->umm,
			cond->state);
	xnfree(cond);
}
