	       ...
	       (int fd, struct user_msghdr __user *umsg, int flags))
{
	...
	return steely_copy_to_user(umsg, &m, sizeof(*umsg)) ?: ret;
}

STEELY_SYSCALL(sendmsg, handover,
	       (int fd, struct user_msghdr __user *umsg, int flags))
{
	struct user_msghdr m;
	int ret;
	        ...
	        void __user **u_addrp))
{
	...
	ret = steely_copy_from_user(&rma, u_rma, sizeof(rma));
	if (ret)
		return ret;
	...
	return steely_copy_to_user(u_addrp, &u_addr, sizeof(u_addr));
}
		...
		struct timeval __user *u_tv))
{
	...
	if (u_tv) {
		if (xnthread_test_localinfo(curr, XNSYSRST)) {
			...
			if (!access_wok(u_tv, sizeof(tv))
			    || steely_copy_from_user(&tv, u_tv, sizeof(tv)))
				return -EFAULT;
	...
	for (i = 0; i < XNSELECT_MAX_TYPES; i++)
		if (ufd_sets[i]) {
			...
			if (!access_wok((void __user *) ufd_sets[i],
					sizeof(fd_set))
			    || steely_copy_from_user(in_fds[i],
	...
	if (!selector) {
		...
		if ((err = __steely_select_bind_all(selector, in_fds, nfds)))
			return err;
	}
...
out:
	if (u_tv && (err > 0 || err == -EINTR)) {
		...
		if (steely_copy_to_user(u_tv, &tv, sizeof(tv)))
			return -EFAULT;
	}
	...
	if (err >= 0)
		for (i = 0; i < XNSELECT_MAX_TYPES; i++)
			if (ufd_sets[i]
			    && steely_copy_to_user((void __user *) ufd_sets[i],
