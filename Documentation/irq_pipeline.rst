...
in the hardware, before running out-of-band handlers for that event if
any.

.. _Holding interrupt lines:
In absence of out-of-band handler for the event, when the interrupt
controller keeps asserting the interrupt signal until the cause has
been lifted in the device, the flow handlers as modified by the
pipelining code may mask the interrupt line until the in-band handler
has run from the root stage. This typically happens with
level-triggered interrupts. This addresses the following scenario,
which happens for a similar reason while an IRQ thread waits for being
...
Since the IRQ handlers sharing an interrupt line are either in-band or
out-of-band in a mutually exclusive way, such masking cannot delay
out-of-band events.
...
:c:func:`generic_pipeline_irq` invokes it immediately, after switching
...
- Generic IRQ core
  ...
  * IRQ descriptor management.
    ...
    routine. Likewise, :c:func:`generic_handle_irq` detects IRQ
    chaining and notifies the pipelining logic about cascaded IRQs.
...
- Drivers
  ...
  * IRQ chip drivers
    ...
    handlers by acquiring the mutable per-IRQ `irq_desc::lock`
    spinlock.  In addition, there might be inner spinlocks defined by
    some ``irqchip`` drivers for serializing handlers running on
    different CPUs when accessing a common hardware for dealing with
    *distinct* IRQs. Adapting the `irqchip` driver to support
