...
#define hard_lock_acquire(__rlock, __try)				\
	do {								\
		if (irq_pipeline_debug_locking())			\
			spin_acquire(&(__rlock)->dep_map, 0, __try, _RET_IP_); \
	} while (0)

#define hard_lock_release(__rlock)					\
	do {								\
		if (irq_pipeline_debug_locking())			\
			spin_release(&(__rlock)->dep_map, 1, _RET_IP_);	\
	} while (0)
...
#define hard_spin_lock_init(__rlock)				\
	do {							\
		...
		__raw_spin_lock_init(__rlock, #__rlock, &__key);\
	} while (0)
#else
#define hard_spin_lock_init(__rlock)				\
