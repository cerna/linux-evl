...
#if defined(CONFIG_STEELY_TIMER_RBTREE)
...
#endif /* CONFIG_STEELY_TIMER_LIST */
...
static inline void xntimer_set_clock(struct xntimer *timer,
				     struct xnclock *newclock)
{
	STEELY_BUG_ON(STEELY, newclock != &nkclock);
}
...
#if defined(CONFIG_STEELY_EXTCLOCK) && defined(CONFIG_STEELY_STATS)
