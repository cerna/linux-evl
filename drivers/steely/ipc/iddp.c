...
static void iddp_close(struct rtdm_fd *fd)
{
	...
	if (test_bit(_IDDP_BOUND, &sk->status)) {
		...
		if (sk->name.sipc_port > -1) {
			steely_atomic_enter(s);
			xnmap_remove(portmap, sk->name.sipc_port);
			steely_atomic_leave(s);
		}
		if (sk->bufpool != &steely_heap) {
			    ...
			    const struct user_msghdr *msg, int flags)
{
	...
	if (msg->msg_name) {
		...
		if (daddr.sipc_port < 0 ||
		    daddr.sipc_port >= CONFIG_STEELY_IDDP_NRPORT)
			return -EINVAL;
	} else {
			      ...
			      struct sockaddr_ipc *sa)
{
	...
	if (sa->sipc_port < -1 ||
	    sa->sipc_port >= CONFIG_STEELY_IDDP_NRPORT)
		return -EINVAL;
...
{
	portmap = xnmap_create(CONFIG_STEELY_IDDP_NRPORT, 0, 0);
	if (portmap == NULL)
		return -ENOMEM;
