			    ...
			    const struct user_msghdr *msg, int flags)
{
	...
	if (msg->msg_name) {
		...
		if (daddr.sipc_port < 0 ||
		    daddr.sipc_port >= CONFIG_STEELY_BUFP_NRPORT)
			return -EINVAL;
	} else {
			      ...
			      struct sockaddr_ipc *sa)
{
	...
	if (sa->sipc_port < -1 ||
	    sa->sipc_port >= CONFIG_STEELY_BUFP_NRPORT)
		return -EINVAL;
...
{
	portmap = xnmap_create(CONFIG_STEELY_BUFP_NRPORT, 0, 0);
	if (portmap == NULL)
		return -ENOMEM;
