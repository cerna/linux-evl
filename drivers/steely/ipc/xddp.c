...
static struct rtdm_fd *portmap[CONFIG_STEELY_PIPE_NRDEV]; /* indexes RTDM fildes */
			    ...
			    const struct user_msghdr *msg, int flags)
{
	...
	if (msg->msg_name) {
		...
		if (daddr.sipc_port < 0 ||
		    daddr.sipc_port >= CONFIG_STEELY_PIPE_NRDEV)
			return -EINVAL;
	} else {
			      ...
			      struct sockaddr_ipc *sa)
{
	...
	if (sa->sipc_port < -1 ||
	    sa->sipc_port >= CONFIG_STEELY_PIPE_NRDEV)
		return -EINVAL;
