...
/*
 * We have limited support for global variables: they must be static (otherwise
 * the .rel.got section needs to be initialized), and not have an initializer
 * (otherwise a .data section is needed). Both these limitations could be
 * lifted, but this would complicate somewhat the kernel-side implementation of
 * the vdso, and is not currently needed.
 */
static u32 current_cs_type_and_seq;
static struct clksrc_info clksrc_info[ARM_CLOCK_USER_MMIO_BASE + CLKSRC_USER_MMIO_MAX];
...
static inline notrace bool clock_accessible(void)
{
	return to_type(current_cs_type_and_seq) != ARM_CLOCK_NONE;
}

static notrace u64 read_cycles(void)
{
	const struct clksrc_info *info;
	unsigned cs;

	cs = to_type(ACCESS_ONCE(current_cs_type_and_seq));
	info = &clksrc_info[cs];
...
void vdso_map_clock(const struct vdso_data *vdata, u32 seq, u32 new_type_and_seq)
{
	vdso_read_cycles_fn *read_cycles;
	...
	ACCESS_ONCE(current_cs_type_and_seq) = new_type_and_seq;
...
static notrace u32 vdso_read_begin(const struct vdso_data *vdata)
{
	...
	for (;;) {
		...
		cs_type_and_seq = ACCESS_ONCE(current_cs_type_and_seq);
		if (likely(to_seq(cs_type_and_seq) == to_seq(vdata->cs_type_and_seq)))
			return seq;

		vdso_map_clock(vdata, seq, vdata->cs_type_and_seq);
	}
}
