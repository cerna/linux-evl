...
static void startup_legacy_irq(unsigned int irq)
{
	unsigned long flags;
...
atomic_t irq_mis_count;
	...
	(defined(CONFIG_IRQ_PIPELINE) && defined(CONFIG_SMP))
...
static void ioapic_ack_level(struct irq_data *irq_data)
{
	...
	/*
	 * If pipelining, we must prevent low priority IRQs grabbed
	 * from the head stage from being delayed, waiting for a high
	 * priority interrupt handler running over the root stage to
	 * complete.  This code assumes hard irqs off.
	 */
	if (irqs_pipelined()) {
		i = cfg->vector;
